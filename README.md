...
- 在插件配置面板中填写 `spotify_client_id` 与 `spotify_client_secret`
- 使用 `Spotify点歌 <歌名>` 或 `spotify点歌 <歌名>` 搜索 Spotify 元数据
- Spotify Web API 不提供直接音频流，因此本插件对 Spotify 结果支持卡片与文本外链发送，不支持语音/文件
...
1. card（卡片，网易云为原生卡片，Spotify 为图片卡片）
2. record（语音）
3. file（文件）
4. text（文本链接）
...
| --------------------- | ---------------- |
| 点歌 `<歌名>`         | 使用默认平台点歌 |
| Spotify点歌 `<歌名>`  | 用 Spotify 搜索并发送卡片/外链 |
| `<平台名>点歌 <歌名>` | 指定平台点歌     |
| 点歌 `<歌名> <序号>`  | 直接选择搜索结果 |
| 查歌词 `<歌名>`       | 查询并发送歌词   |
